...
export function buildKoraJournal(
  ...
  const dayNames = language === 'en' ? DAY_NAMES_EN : DAY_NAMES_ES;
  const bestName = dayNames[new Date(`${bestDay.date}T00:00:00`).getDay()] ?? (language === 'en' ? 'a day' : 'un d?a');
  const weakName = dayNames[new Date(`${weakDay.date}T00:00:00`).getDay()] ?? (language === 'en' ? 'a day' : 'un d?a');
