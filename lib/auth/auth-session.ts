...
import { MANAGED_GUEST_NAME, normalizeManagedGuestName } from '@/lib/guest-auth';
...
export function getUserDisplayName(user: Pick<User, 'email' | 'user_metadata'>): string {
  ...
  const managedGuestName = normalizeManagedGuestName(metadataName);
  if (managedGuestName) return managedGuestName;
...
/**
 * Ensure guest profile exists in database
 * Called when creating a guest session
 */
export async function ensureGuestProfileExists(guestUserId: string): Promise<UserProfile | null> {
  const locale = detectLocale();
  const nowIso = new Date().toISOString();
  ...
  try {
    const result = await supabase
      .from('profiles')
      .upsert(guestProfile, { onConflict: 'id' })
      .select('*')
      .maybeSingle();

    if (result.error) {
      console.error('Error creating guest profile:', result.error);
      return null;
    }

    return (result.data as UserProfile | null) ?? null;
  } catch (error) {
...
export async function resolvePostAuthRouteFromCurrentUser() {
  ...
  await saveOnboardingProgress(Routes.auth.onboarding.transition, {
    name: getUserDisplayName(user),
    email: user.email ?? '',
  });

  return {
    route: Routes.auth.onboarding.transition,
    profile,
  };
}
