...
async function getCoords(): Promise<{ latitude: number; longitude: number } | null> {
  // Avoid surprising system prompts when hydration widgets mount on home.
  const permission = await Location.getForegroundPermissionsAsync().catch((e) => {
    console.debug?.('[water-weather] getForegroundPermissionsAsync failed', e);
    return null;
  });
