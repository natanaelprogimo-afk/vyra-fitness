...
export async function readTodayStepsFromHealthConnect(options?: {
  ...
  try {
    ...
    // Read only today's steps - from UTC midnight today to UTC midnight tomorrow
    const today = new Date();
    const startOfDay = new Date(Date.UTC(today.getUTCFullYear(), today.getUTCMonth(), today.getUTCDate(), 0, 0, 0, 0));
    const startOfTomorrow = new Date(startOfDay);
    startOfTomorrow.setUTCDate(startOfTomorrow.getUTCDate() + 1);
    const snapshot = await readStepsSnapshot(startOfDay.toISOString(), startOfTomorrow.toISOString());
