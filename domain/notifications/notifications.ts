...
function resolveAndroidChannelId(type: NotifType): string {
  if (type === 'fasting_start' || type === 'fasting_phase' || type === 'fasting_complete' || type === 'fasting_complete_soon') return 'vyra-fasting';
  if (type === 'context_proactive') return 'vyra-context';
  if (type === 'female_prediction') return 'vyra-context';
  if (type === 'streak_at_risk' || type === 'sleep_smart_alarm' || type === 'workout_rest_complete') return 'vyra-important';
...
function buildNotificationActions(type: NotifType): Notifications.NotificationAction[] {
  ...
  if (type === 'workout_rest_complete') {
    return [
      {
        identifier: NOTIFICATION_ACTIONS.openWorkout,
        buttonTitle: 'Volver al set',
      },
    ];
  }
  ...
  if (type === 'female_prediction') {
    return [
      {
        identifier: NOTIFICATION_ACTIONS.openFemale,
        buttonTitle: 'Abrir ciclo',
      },
    ];
  }
  ...
  if (type === 'sleep_morning_log' || type === 'sleep_smart_alarm') {
    return [
      {
        identifier: NOTIFICATION_ACTIONS.openSleepLog,
        buttonTitle: type === 'sleep_smart_alarm' ? 'Abrir sueño' : 'Registrar ahora',
      },
    ];
  }
...
export async function scheduleSleepMorningLog(hour = 7, minute = 30): Promise<void> {
  await cancelNotifsByType('sleep_morning_log');
  await scheduleNotif({
    id: 'sleep_morning_log_daily',
    type: 'sleep_morning_log',
    title: 'Registro rápido de sueño',
    body: 'Registra anoche en un flujo corto y deja el resto del día mejor calibrado.',
    trigger: {
      type: 'daily',
      hour,
      minute,
    ...
    data: {
      action: NOTIFICATION_ACTIONS.openSleepLog,
      quick: true,
      source: 'sleep_morning_log',
      deepLink: 'vyrafitness://sleep-quick-log?source=notification&quick=true',
    },
  });
}
...
export async function scheduleFastingStartReminder(hour: number, minute: number): Promise<void> {
  await cancelNotifsByType('fasting_start');
  await scheduleNotif({
    id:    `fasting_start_${hour}_${minute}`,
    type:  'fasting_start',
    title: 'Tu ventana de ayuno ya puede empezar',
    body:  'Abre Vyra cuando quieras y arranca el timer con un toque.',
    trigger: {
      type: 'daily',
      hour,
      minute,
...
export async function scheduleFastingAlmostCompleteNotif(
  triggerDate: Date,
  hoursLeft = 2,
): Promise<void> {
  await scheduleNotif({
    id:    `fasting_complete_soon_${triggerDate.getTime()}`,
    type:  'fasting_complete_soon',
    title: `Quedan ${hoursLeft}h para completar tu ayuno`,
    body:  'Si te sientes bien, esta es una buena franja para cerrar fuerte y abrir la comida con mas claridad.',
    trigger: { date: triggerDate } as Notifications.NotificationTriggerInput,
  });
}
...
export async function scheduleFemalePredictionAlerts(
  alerts: Array<{
    id: string;
    title: string;
    body: string;
  ...
  for (const alert of alerts) {
    if (!(alert.triggerDate instanceof Date) || Number.isNaN(alert.triggerDate.getTime())) continue;
    if (alert.triggerDate.getTime() <= Date.now() + 60 * 1000) continue;

    await scheduleNotif({
      id: alert.id,
      type: 'female_prediction',
      title: alert.title,
      body: alert.body,
      trigger: { date: alert.triggerDate } as Notifications.NotificationTriggerInput,
      data: {
        action: NOTIFICATION_ACTIONS.openFemale,
        source: 'female_prediction',
        symptom: alert.symptom ?? '',
        nextDateStart: alert.nextDateStart ?? '',
      },
    });
  }
}
...
function isMinuteInsideWindow(candidate: number, start: number, end: number) {
  if (start <= end) {
...
export function computeSleepSmartAlarmMinute(
  bedtimeMinute: number,
  windowStartMinute: number,
  windowEndMinute: number,
): { minute: number; cycleCount: number | null; fallbackUsed: boolean } {
  const safeBedtime = normalizeMinutesOfDay(bedtimeMinute);
  const safeStart = normalizeMinutesOfDay(windowStartMinute);
  const safeEnd = normalizeMinutesOfDay(windowEndMinute);
  const candidates = [4, 5, 6].map((cycleCount) => ({
    cycleCount,
    minute: normalizeMinutesOfDay(safeBedtime + cycleCount * 90 + 14),
  }));
  const inWindow = candidates.filter((item) => isMinuteInsideWindow(item.minute, safeStart, safeEnd));

  if (inWindow.length) {
    const windowMidpoint =
      safeStart <= safeEnd
        ? safeStart + (safeEnd - safeStart) / 2
        : normalizeMinutesOfDay(safeStart + ((safeEnd + 1440 - safeStart) % 1440) / 2);
    const best = [...inWindow].sort(
      (left, right) => Math.abs(left.minute - windowMidpoint) - Math.abs(right.minute - windowMidpoint),
    )[0]!;
    return {
      minute: best.minute,
...
export async function scheduleSleepSmartAlarm(input: {
  bedtimeMinute: number;
  windowStartMinute: number;
  windowEndMinute: number;
}): Promise<{ minute: number; cycleCount: number | null; fallbackUsed: boolean }> {
  await cancelNotifsByType('sleep_smart_alarm');

  const plan = computeSleepSmartAlarmMinute(
    input.bedtimeMinute,
    input.windowStartMinute,
    input.windowEndMinute,
  );
  const hour = Math.floor(plan.minute / 60);
  const minute = plan.minute % 60;

  await scheduleNotif({
    id: 'sleep_smart_alarm_daily',
    type: 'sleep_smart_alarm',
    title: 'Despertar inteligente',
    body: plan.fallbackUsed
      ? 'Si no detectamos un cierre limpio de ciclo, te despertamos al final de tu ventana.'
      : `Intentaremos despertarte cerca del final de tu ciclo ${plan.cycleCount ?? 5} para que arranques mejor.`,
    trigger: {
      type: 'daily',
      hour,
      minute,
...
export async function scheduleWorkoutRestCompleteNotification(
  triggerDate: Date,
  copy?: { exerciseName?: string | null; nextExerciseName?: string | null },
): Promise<void> {
  await cancelNotifsByType('workout_rest_complete');
  if (triggerDate.getTime() <= Date.now()) return;

  const focusName = copy?.nextExerciseName?.trim() || copy?.exerciseName?.trim() || 'tu siguiente set';
  await scheduleNotif({
    id: 'workout_rest_complete_active',
    type: 'workout_rest_complete',
    title: 'Descanso listo',
    body: `Vuelve a ${focusName}.`,
    trigger: { date: triggerDate } as Notifications.NotificationTriggerInput,
    data: {
      action: NOTIFICATION_ACTIONS.openWorkout,
      source: 'workout_rest_complete',
    },
  });
}

export async function cancelWorkoutRestCompleteNotification(): Promise<void> {
  await cancelNotifsByType('workout_rest_complete');
}
