...
export default function Step5Premium() {
  ...
  const buildOnboardingData = (): OnboardingData => {
    ...
    const activeModules = (params.active_modules ?? '')
      .split(',')
    ...
    return {
      ...
      active_modules: activeModules.length > 0 ? activeModules : undefined,
      terms_accepted: true,
  ...
  const handleStartPremium = async () => {
    const ok = await saveOnboarding(buildOnboardingData());
    if (ok) {
      router.replace({ pathname: '/premium/paywall', params: { trigger: 'organic', plan } } as any);
    }
  };
    ...
    <SafeScreen>
      <ScrollView showsVerticalScrollIndicator={false}>
        <View style={styles.header}>
          ...
          <Text style={styles.title}>Desbloquea{'\n'}Vyra Premium</Text>
          <Text style={styles.subtitle}>
            Pago seguro con PayPal. Cancelas la renovacion cuando quieras desde tu perfil.
          </Text>
        </View>
        ...
        <View style={styles.plans}>
          ...
          <Pressable
            ...
            style={[styles.planCard, styles.planCardYearly, plan === 'yearly' && styles.planCardActive]}
          >
        ...
        <View style={styles.ctas}>
          ...
          >
            Ir al checkout de PayPal
          </Button>
          <Button onPress={handleContinueFree} variant="ghost" fullWidth loading={isLoading}>
            Continuar gratis (con anuncios)
          </Button>
          <Text style={styles.legalNote}>
            El cobro se procesa al confirmar en PayPal para el plan {plan === 'yearly' ? 'anual $99.99' : 'mensual $12.99'}.
            Puedes cancelar la renovacion en cualquier momento desde tu perfil.
          </Text>
        </View>
      </ScrollView>
    </SafeScreen>
