...
const SLEEP_TIPS = [
  { id: 'consistency', emoji: '⏰', tip: 'Mantén un horario fijo de sueño, incluso los fines de semana.' },
...
export default function SleepInsightsScreen() {
  ...
  const recoveryIndex = last3.length
    ...
    : 0;
  const recoveryColor = recoveryIndex >= 80 ? Colors.steps : recoveryIndex >= 60 ? Colors.fasting : Colors.warning;
  ...
  const { data: correlationData } = useQuery({
    ...
    queryFn: async () => {
      ...
      const sleepByDate = Object.fromEntries((sleepRes.data ?? []).map((entry) => [entry.end_time.split('T')[0], entry]));
      const stepsByDate = Object.fromEntries((stepsRes.data ?? []).map((entry) => [entry.logged_date, entry.steps]));
    ...
    <SafeScreen>
      ...
      <ScrollView showsVerticalScrollIndicator={false}>
        <Card style={styles.readinessCard}>
          <Text style={styles.readinessTitle}>Índice de descanso</Text>
          <View style={styles.readinessRow}>
            ...
            <View style={styles.readinessInfo}>
              <Text style={[styles.readinessLabel, { color: recoveryColor }]}>
                {recoveryIndex >= 80 ? 'Listo para rendir' : recoveryIndex >= 60 ? 'Bien recuperado' : 'Necesitas descansar'}
              </Text>
              <Text style={styles.readinessDesc}>Basado en las últimas 3 noches</Text>
            </View>
          </View>
          <ProgressBar value={recoveryIndex} color={recoveryColor} height={8} animated showPct={false} style={styles.readinessBar} />
          ...
          <View style={styles.subScores}>
            {[
              ...
              {
                label: 'Consistencia',
                pct: history.filter((entry) => entry.duration_min / 60 >= goalHours).length / Math.max(1, history.length) * 100,
                color: Colors.brand,
              },
            ].map((item) => (
              <View key={item.label} style={styles.subScoreItem}>
                <View style={styles.subScoreHeader}>
                  ...
                  <Text style={[styles.subScoreValue, { color: item.color }]}>{Math.round(item.pct)}%</Text>
                </View>
        ...
        <PremiumLock feature="Correlación sueño-rendimiento" trigger="sleep_insights">
          <Card style={styles.correlationCard}>
            ...
            {correlationData && correlationData.length > 0 ? (
                ...
                {correlationData.slice(-7).map((entry, index) => (
                  <View key={index} style={styles.correlationRow}>
                    <Text style={styles.correlationDate}>
                      {new Date(`${entry.date}T12:00:00`).toLocaleDateString('es', { weekday: 'short', day: 'numeric' })}
                    </Text>
                    <View style={styles.correlationBars}>
                      <View style={[styles.corrBar, { width: `${entry.sleep}%`, backgroundColor: Colors.sleep }]} />
                      <View style={[styles.corrBar, { width: `${Math.min(100, entry.steps / 150)}%`, backgroundColor: Colors.steps }]} />
                    </View>
                  </View>
                ))}
              ...
              <Text style={styles.correlationEmpty}>Necesitas al menos 7 días de datos para ver esta correlación.</Text>
            )}
          </Card>
        </PremiumLock>
