...
export default function NutritionHistoryScreen() {
  ...
  const maxKcal = Math.max(...weeklyData.map((item) => item.calories), calorieGoal);
  const avgCalories = weeklyData.length ? weeklyData.reduce((sum, item) => sum + item.calories, 0) / weeklyData.length : 0;
  const avgProtein = weeklyData.length ? weeklyData.reduce((sum, item) => sum + item.protein, 0) / weeklyData.length : 0;
  const avgCarbs = weeklyData.length ? weeklyData.reduce((sum, item) => sum + item.carbs, 0) / weeklyData.length : 0;
  const avgFat = weeklyData.length ? weeklyData.reduce((sum, item) => sum + item.fat, 0) / weeklyData.length : 0;
  ...
  return (
    <SafeScreen padHorizontal={false} padBottom>
      ...
      <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={styles.content}>
          ...
          {weeklyData.length > 0 ? (
            <View style={styles.bars}>
              {weeklyData.map((item, index) => {
                ...
                const onTarget = item.calories >= calorieGoal * 0.85 && item.calories <= calorieGoal * 1.15;
                const over = item.calories > calorieGoal * 1.15;
                const dayLabel = new Date(`${item.date}T12:00:00`).toLocaleDateString('es', { weekday: 'short' });
                const isToday = item.date === new Date().toISOString().split('T')[0];

                return (
                  <View key={index} style={styles.barCol}>
                    <Text style={styles.barVal}>
                      {item.calories >= 1000 ? `${(item.calories / 1000).toFixed(1)}k` : Math.round(item.calories)}
                    </Text>
                    <View style={styles.barTrack}>
                      <View style={[styles.goalLine, { bottom: `${(calorieGoal / maxKcal) * 100}%` }]} />
                      <View
                        style={[
                          styles.barFill,
                          {
                            height: `${pct}%`,
                            backgroundColor: over ? Colors.error : onTarget ? Colors.nutrition : `${Colors.nutrition}55`,
                          },
                        ]}
                      />
                    </View>
                    <Text style={[styles.barDay, isToday && { color: Colors.nutrition, fontFamily: FontFamily.bold }]}>
        ...
        <Card>
          <MacroRow label="Proteínas" avg={Math.round(avgProtein)} goal={macroGoals.protein} color="#FF6B6B" />
          <MacroRow label="Carbohidratos" avg={Math.round(avgCarbs)} goal={macroGoals.carbs} color={Colors.fasting} />
          <MacroRow label="Grasas" avg={Math.round(avgFat)} goal={macroGoals.fat} color="#FFD43B" />
        </Card>
      </ScrollView>
    </SafeScreen>
  );
