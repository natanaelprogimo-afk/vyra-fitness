...
function bmiToAngle(bmi: number): number {
  ...
  return ((clamped - 10) / (45 - 10)) * 180 - 90; // -90 a 90 grados
}
...
export function BmiGauge({ bmi, category }: BmiGaugeProps) {
  ...
  const categoryColor =
    BMI_RANGES.find((range) => bmi >= range.min && bmi < range.max)?.color ??
    Colors.error;
      ...
      <View style={styles.legend}>
        {BMI_RANGES.map((range) => (
          <View key={range.label} style={styles.legendItem}>
            ...
            <Text style={[styles.legendText, bmi >= range.min && bmi < range.max && styles.legendActive]}>
              {range.label}
            </Text>
          </View>
        ))}
